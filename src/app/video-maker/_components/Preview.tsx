...
function AudioTrackPlayer({ track }: { track: Track }) {
  ...
  useEffect(() => {
    ...
    const currentSrc = audio.getAttribute('data-media-id');

    if (currentSrc !== media.id) {
      audio.setAttribute('data-media-id', media.id);
      ...
      if (!state.isPlaying) {
        ...
        if (!audio.seeking && Math.abs(audio.currentTime - sourceTime) > 0.25) {
          audio.currentTime = sourceTime;
        }
      }
    }
  ...
  return (
    <div className="flex flex-col items-center gap-3 h-full py-4 px-6">
      ...
      <div className="flex w-full max-w-2xl items-center gap-3">
        <span className="w-10 text-right font-mono text-xs text-slate-500 tabular-nums">
          {formatTime(state.playhead)}
        </span>

        {/* Seek bar */}
        <div
          className="flex-1 h-2 bg-slate-200 rounded-full cursor-pointer relative overflow-hidden"
        ...
        <span className="w-10 font-mono text-xs text-slate-400 tabular-nums">
      ...
      <button
        ...
        className="flex h-10 w-10 items-center justify-center rounded-full bg-violet-600 text-white shadow-md transition hover:bg-violet-700 active:scale-95 disabled:opacity-40"
      >
        {state.isPlaying ? (
