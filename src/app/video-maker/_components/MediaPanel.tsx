    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 border-b border-slate-200 flex flex-col gap-2">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-violet-300 bg-violet-50 px-3 py-3 type-level-2 text-violet-700 transition hover:border-violet-400 hover:bg-violet-100"
        >
          <svg
      ...
      <div className="flex border-b border-slate-200">
        {(['videos', 'audio'] as const).map((t) => (
          <button
            ...
            className={`flex-1 py-3 type-level-2 transition capitalize ${
              tab === t
      ...
      {!activeProject && (
        <div className="mx-4 mt-3 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2">
          <p className="type-level-3 text-amber-700">
      ...
      <div className="flex-1 overflow-y-auto p-3">
        {visible.length === 0 && (
          <p className="py-8 text-center type-level-3 text-slate-400">
            No {tab} uploaded yet.
          </p>
        )}
  ...
  return (
    ...
    >
      ...
      <div className="px-2 py-1.5">
        <p className="truncate type-level-3 text-slate-700">{item.name}</p>
        <p className="type-level-4 text-slate-400">{formatSize(item.size)}</p>
      </div>
